...
compute_target_counts <- function(p_neg, Tpos, Tneg){
  ...
  data.table(p_neg)[, let(
    n_pos = ifelse(
      p_neg<0.5,
      as.integer(n_small*(1-p_neg)/p_neg),
      n_small),
    n_neg = ifelse(
      p_neg<0.5,
      n_small,
      as.integer(n_small*p_neg/(1-p_neg)))
  )][
  , n_imb := n_pos + n_neg
  ][
  , N_pos_neg := as.integer(n_imb/2)
  ][, let(
    pos = n_pos + N_pos_neg,
    neg = n_neg + N_pos_neg,
    check_prop = n_neg/n_imb
...
Tpos <- ltab[1]
Tneg <- ltab[2]
(Laribi_grid_dt <- data.table(
  p_neg
)[, let(
  n_neg_max=2*Tneg*p_neg/(2*p_neg+1),
  n_pos_max=2*Tpos*(1-p_neg)/(3-2*p_neg)
)][, let(
  n_neg_max_pos=n_neg_max*(1-p_neg)/p_neg,
  n_pos_max_neg=n_pos_max*p_neg/(1-p_neg)
)][, let(
  n_neg_max_N=(n_neg_max+n_neg_max_pos)/2,
  n_pos_max_N=(n_pos_max+n_pos_max_neg)/2
)][, let(
  n_neg_max_D=Tpos-n_neg_max_N-n_neg_max_pos,
  n_pos_max_D=Tneg-n_pos_max_N-n_pos_max_neg
)][])
Laribi_grid_dt[100]
(Laribi_grid_dt <- compute_target_counts(p_grid, Tpos, Tneg)[is.finite(check_prop)])
