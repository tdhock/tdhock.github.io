...
instantiated resampling. To use these data with targets, we first save
them to a file on disk.

```{r}
save(target_dt, bench.grid, file="2025-05-20-mlr3-targets-in.RData")
```
...
targets::tar_script({
  ...
  compute <- function(target_num){
    ...
    for(set_name in c("train","test")){
      attr_name <- paste0(set_name, "_set")
      set_rows[[set_name]] <- bench.grid[[attr_name]][[target_row$bench_row]][[target_row$iteration]]
    }
    L$train(this_task, set_rows$train)
    pred <- L$predict(this_task, set_rows$test)
    bench.grid$resampling[[target_row$bench_row]]$instance$iteration.dt[
...
targets::tar_script({
  load("2025-05-20-mlr3-targets-in.RData")
  library(data.table)
  compute <- function(it_i){
    target_row <- target_dt[it_i]
    bench_row <- bench.grid[target_row$bench_row]
    L <- bench_row$learner[[1]]
    this_task <- bench_row$task[[1]]
    set_rows <- list()
    for(set_name in c("train","test")){
      attr_name <- paste0(set_name, "_set")
      set_rows[[set_name]] <- bench_row[[attr_name]][[1]][[target_row$iteration]]
    }
    L$train(this_task, set_rows$train)
    L$predict(this_task, set_rows$test)
  }
  job_targs <- tarchetypes::tar_map(
    list(iteration=seq_len(nrow(target_dt))),
    targets::tar_target(result, compute(iteration)))
  list(
    tarchetypes::tar_combine(combine, job_targs, command=rbind(!!!.x)),
    job_targs)
})
