...
The Pelt Algorithm of Killick et al allows us to prune the set of
change-points:
...
data.table(change=pelt.change.vec, cost=pelt.cost.vec[-1], candidates=pelt.candidates.vec)
all.equal(pelt.cost.vec[-1], best.cost.vec)
all.equal(best.change.vec, pelt.change.vec)
plot(pelt.candidates.vec)
```
