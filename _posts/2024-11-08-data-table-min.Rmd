...
OPART <- function(sim.mat, penalty){
  ...
  out.dt <- data.table(
    change=rep(NA_integer_, nrow(sim.mat)),
    cost=rep(NA_real_, nrow(sim.mat)))
  for(up.to in seq(1, nrow(sim.mat))){
    last.seg.cost <- rowSums(cost_trick(1:up.to, rep(up.to, up.to)))
    change.cost <- if(up.to>1)out.dt$cost[seq(1, up.to-1)]+penalty
    other.seg.cost <- c(0, change.cost)
    total.cost <- other.seg.cost+last.seg.cost
    best.i <- which.min(total.cost)
    best.cost <- total.cost[best.i]
    set(out.dt, up.to, "change", best.i)
    set(out.dt, up.to, "cost", best.cost)
  }
  list(out.dt, max.candidates=up.to)
}
```
...
```{r}
opart_vs_pelt <- atime::atime(
  N=unique(as.integer(10^seq(1,3,by=0.2))),
  setup={
    x.mat <- sim.mat[1:N,]
  },
  result=function(L)with(L, data.table(max.candidates)),
  seconds.limit=0.1,
