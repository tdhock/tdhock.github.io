...
import torch
import pandas as pd
def ROC(pred_tensor, label_tensor):
...
def AUM(pred_tensor, label_tensor):
    ...
    """
    roc = ROC(pred_tensor, label_tensor)
    return torch.sum(roc["min"][1:-1] * roc["min_thresh"][1:].diff())

def AUC(pred_tensor, label_tensor):
    roc = ROC(pred_tensor, label_tensor)
...
AUC(pred_vec, label_vec)
ROC(pred_vec, label_vec)

roc_wide = ROC(pred_vec, label_vec)
roc_long = pd.melt(edf, value_vars=["fp","fn","min"], id_vars=["min_thresh","max_thresh"])
