...
add.space.between.double.curly.braces <- function(x){
  ## this is only necessary in the blog, to avoid jekyll error
  ## Liquid Exception: …was not properly terminated with regexp: /\}\}/ in /github/workspace/_posts/2026-09-22-find-bib-issues.md
  gsub("\\{\\{", "\\{ \\{", x)
}
(refs.dt <- nc::capture_all_str(
  ...
  nc::before_match(
    ...
    ref="[^,]+",
    ",\n",
    fields="(?s).*?", add.space.between.double.curly.braces,
    "[}](?:$|\n *\n)")))
```
