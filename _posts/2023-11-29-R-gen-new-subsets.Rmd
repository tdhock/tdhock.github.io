...
### Generalization to new groups resampler

The goal of this section is to define a replacement for
`mlr3::Resampling`, which will interpret group and stratum variables
differently, in order to create cross-validation experiments that
will be able to compare models trained on same/other/all group(s).

```{r}
MyResampling = R6::R6Class("MyResampling",
  public = list(
    id = NULL,
    label = NULL,
    param_set = NULL,
  ...
  private = list(
    ...
    .get_set = function(getter, i) {
      ...
      i = checkmate::assert_int(i, lower = 1L, upper = self$iters, coerce = TRUE)
      ids = getter(i)
