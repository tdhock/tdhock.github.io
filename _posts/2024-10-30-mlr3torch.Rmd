...
```{r plot-torch-linear}
n.epochs <- 1000
optimizer <- torch::optim_sgd(seq_linear_model$parameters, lr=0.1)
loss_dt_list <- list()
...
```{r}
n.epochs <- 100
optimizer <- torch::optim_sgd(seq_conv_model$parameters, lr=0.1)
...
```{r}
po_list <- list(
  mlr3pipelines::po("select", selector = mlr3pipelines::selector_type(c("numeric", "integer"))),
  ...
  mlr3pipelines::po("torch_loss", mlr3torch::t_loss("cross_entropy")),
  mlr3pipelines::po("torch_optimizer", mlr3torch::t_opt("sgd", lr=0.01)),
  mlr3pipelines::po("torch_model_classif", batch_size = 32, epochs = 2L))
