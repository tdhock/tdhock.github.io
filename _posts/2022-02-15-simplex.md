...
```
## <ggplot: (-9223372036845816500)>
```

![plot of chunk binary-loss-scores](figure/binary-loss-scores-1.png)
...
```
## <ggplot: (-9223372036741951768)>
```

![plot of chunk binary-loss-prob](figure/binary-loss-prob-3.png)
...
triple of probability values $(p_1,p_2,p_3)$ on the simplex, which
satisfy $\sum_i p_i=1$ and $p_i\geq 0$. To plot the loss function for
each probability triple in the simplex, we can make a heat map by
first computing a grid of (x,y) values:
...
The matrix $A$ is the linear transformation which converts x,y
coordinates to probability triple coordinates. To solve for $A$ we
just need to matrix multiply both sides by the inverse of
\texttt{vertices\_mat},

$$
A =
\texttt{vertices\_mat}^{-1}
\begin{bmatrix}
1 & 0 & 0 \\
0 & 1 & 0 \\
0 & 0 & 1
...
So we just need to invert that matrix:
...
def gg_loss(loss_name, grid_loss_df, breaks=[0,0.5,1]):
    return p9.ggplot()+\
        ...
        p9.scale_y_continuous(
            breaks=breaks)+\
        p9.coord_equal()
show(gg_loss("Logistic", logistic_df))
```

```
## <ggplot: (-9223372036844534400)>
```

![plot of chunk multi-logistic](figure/multi-logistic-5.png)
...
```python
show(gg_loss("Zero-one", zero_one_df))
```

```
## <ggplot: (-9223372036741950496)>
```

![plot of chunk multi-zero-one](figure/multi-zero-one-7.png)
