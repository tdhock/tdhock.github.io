...
In other fields of science (besides CS), there are other names for these sets.
In medicine, we often use "holdout" set instead of "test" set.

### Laribi 2024

[Laribi 2024](https://www.medrxiv.org/content/medrxiv/early/2024/07/26/2024.06.21.24309191.full.pdf) Figure 2 divides Dataset into Learning set and Holdout set. Learning set is divided into Training set and Testing set. These names are potentially confusing, so here is a table to clarify:

| ML         | Laribi 2024 | purpose                                                               |
|------------|-------------|-----------------------------------------------------------------------|
| full       | Dataset     | all available data                                                    |
| train      | Learning    | training (including hyper-parameter selection)                        |
| test       | Holdout     | Evaluation (after hyper-parameter selection and possibly re-training) |
| subtrain   | Training    | computing gradients / training for one hyper-parameter                |
| validation | Testing     | Hyper-parameter selection, to avoid over- and under-fitting           |
