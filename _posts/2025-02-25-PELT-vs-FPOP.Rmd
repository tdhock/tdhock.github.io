...
## Verification of PELT with linear changes

```{r}
library(data.table)
N_data_vec <- c(c(1,2,4,8)*1e3,c(1,2,4,8)*1e4)
sim_data_list <- list()
sim_changes_list <- list()
for(N_data in N_data_vec){
  for(simulation in names(sim_fun_list)){
...
PELT <- function(sim.mat, penalty, prune=TRUE, verbose=FALSE){
  if(!is.matrix(sim.mat))sim.mat <- cbind(sim.mat)
  print(cum.data <- rbind(0, apply(sim.mat, 2, cumsum)))
  sum_trick <- function(m, start, end)m[end+1,,drop=FALSE]-m[start,,drop=FALSE]
  cost_trick <- function(start, end){
    sum_vec <- sum_trick(cum.data, start, end)
    N <- end+1-start
...
algo.prune <- c(
  ...
  PELT=TRUE)
for(N_data in N_data_vec){
  for(simulation in "linear_changes"){
    N_sim <- paste(N_data, simulation)
    data_value <- sim_data_list[[N_sim]]$data_value
    for(algo in names(algo.prune)){
      prune <- algo.prune[[algo]]
...
ggplot()+
  ...
  scale_x_log10(
    breaks=seq(0,max(N_data_vec),by=25))
pelt_info[, .(mean.candidates=mean(candidates)), by=.(N_data,simulation)]

pelt_info[, to_end := max(data_index)-data_index, by=N_data]
ggplot()+
  theme_bw()+
  scale_color_manual(values=algo.colors)+
  geom_line(aes(
    to_end, candidates, color=algo),
    data=pelt_info)+
  facet_grid(simulation ~ N_data, labeller=label_both, scales="free_x", space="free")+
  scale_x_continuous(
    limits=c(0,500),
...
, to_end := max(data_index)-data_index, by=N_data
][])
ggplot()+
  theme_bw()+
  geom_tile(aes(
    to_end, change, fill=cost),
    data=both_index[N_data==80000 & to_end<500])+
  scale_fill_gradient(low="black", high="red")+
  facet_grid(simulation ~ N_data, label=label_both)
