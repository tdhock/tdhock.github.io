...
```{r}
diag.band[, min.dist := min(dist)==dist]
ggplot()+
  geom_tile(aes(
    col, row, fill=dist, color=min.dist),
    size=0.5,
    data=diag.band)+
  scale_fill_gradient(low="white", high="red")+
  scale_color_manual(
...
```{r}
diag.band[, to.join := .I %in% which(min.dist)[1] ]
color.values <- c("TRUE"="black", "FALSE"="transparent")
gg.dist <- function(){
  ggplot()+
    ggtitle(paste0("min dist=", diag.band[min.dist==TRUE]$dist[1]))+
    geom_tile(aes(
      col, row, fill=dist,
    ...
    scale_y_reverse("cluster")+
    scale_x_continuous("cluster")
}
gg.dist()
```
...
```{r}
do.join <- function(){
  join.indices <- diag.band[to.join==TRUE, c(row,col)]
  to.remove <- diag.band[, row %in% join.indices | col %in% join.indices]
  old.dt <- diag.band[to.remove]
  old.dt[, other := ifelse(
  ...
  diag.band <<- rbind(
    diag.band[!to.remove, data.table(row, col, dist)],
    new.dt[, data.table(
