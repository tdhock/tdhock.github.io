...
xgboost.learner$param_set$values$eta <- paradox::to_tune(0.01, 1, log=TRUE)
...
(nn.default$id <- sprintf("%d-NN (default)", nn.default$param_set$values$k))
learner.list <- list(
...
bench.score[
  ...
  selected := sapply(learner, function(L)L$tuning_result$eta)
][
...
, algorithm := factor(
    learner_id,
    c("regr.kknn.tuned", nn.default$id, nn1$id, "regr.featureless"))
][]
