...
```{r}
refs.wide[, author_list := strsplit(author, split=" and ")]
subject <- c("Toby Dylan Hocking", "Hocking, Toby Dylan")
family.pattern <- "[A-Z][^ ]+"
given.pattern <- "[^,]+"
nc::capture_first_vec(subject, "^", family=family.pattern, ", ", given=given.pattern, "$", nomatch.error=FALSE)
nc::capture_first_vec(subject, "^", given=given.pattern, " ", family=family.pattern, "$", nomatch.error=FALSE)
...
```{r}
authors[, .(authors_abbrev=paste(show, collapse=", ")), by=ref]
```
